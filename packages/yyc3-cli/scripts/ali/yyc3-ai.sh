...
AI_DIR="/Volume2/YYC/ai-models"
OLLAMA_DIR="/Volume2/YYC/ollama"
NAS_IP="192.168.0.9"
...
services:
  ...
  ollama-1:
    ...
    container_name: yyc3-ollama-1
    restart: unless-stopped
    ports:
      - "11434:11434"
    Volumes:
      - /Volume2/YYC/ollama/models:/root/.ollama
    environment:
      - OLLAMA_HOST=0.0.0.0
      - OLLAMA_ORIGINS=*
    networks:
  ...
  ai-router:
    ...
    ports:
      - "8888:80"
    Volumes:
      - /Volume2/YYC/ai-models/config/nginx.conf:/etc/nginx/nginx.conf
      - /Volume2/YYC/ai-models/logs:/var/log/nginx
    networks:
      - yyc3-network
    depends_on:
      - ollama-1
  ...
  ai-monitor:
    ...
    container_name: yyc3-ai-monitor
    restart: unless-stopped
    ports:
      - "9091:9090"
    Volumes:
      - /Volume2/YYC/ai-models/config/prometheus.yml:/etc/prometheus/prometheus.yml
      - /Volume2/YYC/ai-models/config/rules:/etc/prometheus/rules
    networks:
      - yyc3-network
    command:
  ...
  ai-cache:
    ...
    container_name: yyc3-ai-cache
    restart: unless-stopped
    ports:
      - "6380:6379"
    Volumes:
      - /Volume2/YYC/ai-models/cache:/data
    networks:
      - yyc3-network
...
AI_ROUTER="http://192.168.0.9:8888"
...
check_status() {
    ...
    for port in 11434 11435; do
        if curl -s "http://192.168.0.9:$port" > /dev/null; then
    ...
    echo ""
    echo "Redis 缓存:"
    if redis-cli -h 192.168.0.9 -p 6380 ping > /dev/null 2>&1; then
        echo "  ✅ Redis - 运行中"
    else
        echo "  ❌ Redis - 停止"
    fi
