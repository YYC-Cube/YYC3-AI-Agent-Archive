...
validate_ip_config() {
    ...
    for file in "${files_to_check[@]}"; do
        if [ -f "$ROOT_DIR/$file" ]; then
            local old_ip_count=$(grep -c "192.168.3.45" "$ROOT_DIR/$file" 2>/dev/null || echo 0)
...
$(find "$ROOT_DIR" -name "*.sh" -o -name "*.yml" -o -name "*.ini" -o -name "*.conf" | while read file; do
    if grep -q "192.168.3.45" "$file" 2>/dev/null; then
        echo "⚠️  $file - 包含旧IP地址"
...
provide_fix_suggestions() {
    ...
    echo ""
    echo "1. 批量替换IP地址:"
    echo "   find $ROOT_DIR -type f $$ -name '*.sh' -o -name '*.yml' -o -name '*.ini' $$ -exec sed -i 's/192.168.3.45/$NEW_IP/g' {} +"
    echo ""
    echo "2. 更新域名配置:"
    echo "   # 编辑 FRP 配置文件"
