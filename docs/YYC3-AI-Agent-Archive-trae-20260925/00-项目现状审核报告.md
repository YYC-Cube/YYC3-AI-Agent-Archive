...
## 二十一、P2 池二批：callId crypto / EPIPE / reload sync / fail-open 告警 / 观测封顶 / marketplace Store / CLI 杂项 / .env 重写（2026-09-26 实施）

> 闭环 §4.4 第 6/9/10/12 条与 §7.3 第 3/6/7 项。审计前置说明：half-open 计数与 executor 输出上限经复核**已在 S0-1 顺带修复**（executor.ts:96-111 计数自增、:383-398 单流 1MB），本章不重复实施。

### 21.1 修复清单

| # | 缺陷（原报告） | 修复 |
| - | -------------- | ---- |
| callId 碰撞 | mcp-runtime `call-${Date.now()}`（无后缀分支）与 Math.random 后缀；registry executor Math.random | 两处统一 `crypto.randomBytes(8).toString('hex')` |
| cowagent EPIPE | `proc.stdin.write` 无 error 监听，子进程先退出触发进程级 EPIPE 异常 | `stdin.on('error')` 吞掉，由 close 统一收敛 |
| cowagent 输出无上限 | stdout/stderr 无限累积 | 单流 1MB + `[output truncated]` 标记（同 executor 口径） |
| reload 只增不删 | 磁盘已删技能残留可执行 | `SkillRegistry.clear()`（逐个发 unregistered 事件）+ `LoaderOptions.sync` + `SkillLoader.reload()`；gateway reload 路由改用；默认 load 保持追加语义（测试锁行为） |
| fail-open 静默 | `.catch(()=>{})` 无任何告警 | 连续失败首次 + 每 5 次告警，恢复清零 |
| CORS 默认 `*` | 无法收敛 | `YYC3_CORS_ORIGINS` 白名单 env；未配置保持 `*`（API Key 头认证不携带凭据，兼容存量） |
| logger 无界 | entries 无限增长 | `maxEntries`（默认 1000 环形裁剪；0=不留内存） |
| metrics 序列无界 | 高基数标签分区无限增长 | `MetricsRegistry({ maxSeries })` 默认 10_000：新分区超限丢弃 + 每指标一次告警，既有分区不受影响 |
| marketplace 重启即失 | 插件注册表纯内存 | `store?` 写穿（install/update/activate/deactivate/remove）+ `restore()` + `flushPending()` |
| CLI 端口段冲突 | 3000-3199 全禁与「3030 起」正面冲突 | 3030-3039 放行；3000-3029/3100-3199 仍限用 |
| config --set 死分支 | Commander 双占位符不合法，options.value 永不存在 | `--set <key>=<value>` 解析 + JSON 自动转型 + 用法报错 |
| i18n.js 空文件 | 0 字节无引用 | 删除 |
| init 模板缺依赖 | 模板 require express 但 dependencies 空 | 补 `express ^4.21.0` |
| .env.example 虚构 | Postgres/DB_*/API_PORT=8000/REDIS_HOST 三件套不存在 | 重写 v2.0.0：仅真实消费项 + 消费方速查表 |
| T14/T16 超时矛盾 | jest timeout 90s < 自身断言 180s | 对齐 190s |

### 21.2 变更文件与版本

| 包 | 版本 | 测试 |
| ---- | ---- | ---- |
| `@yyc3/skill-registry` | 1.2.0 → **1.3.0** | 81 → **83** |
| `@yyc3/skill-gateway` | 1.3.0 → **1.4.0** | 63（reload 测试重写为磁盘真实语义） |
| `@yyc3/mcp-runtime` | 1.4.0 → **1.5.0** | 40 |
| `@yyc3/observability` | 1.1.0 → **1.2.0** | 61 → **65** |
| `@yyc3/plugin-marketplace` | 1.0.1 → **1.1.0** | 29 → **32**（🔴→🟡） |
| `yyc3-cli` | 2.0.0 | 62 → **66** |

文件：executor/runtime（registry）、runtime/cowagent-bridge（mcp）、registry/loader/skills 路由/security/gateway（gateway）、logger/metrics/types（observability）、marketplace/types + package.json（pm）、lib/index + bin + 删除 i18n.js（cli）、`.env.example`。

### 21.3 验证结果

| 门禁 | 结果 |
| ---- | ---- |
| 单包 | ✅ 83/83 · 63/63 · 40/40 · 65/65 · 32/32 · 66/66 |
| 中文裸环境全量 | ✅ 24 tasks 全绿，**1197 全绿**（1184 + 13） |
| 英文裸环境全量 | ✅ 24 tasks 全绿（performance 首跑 556ms 超阈值系调试器负载噪声，重跑过） |
| typecheck / build / lint | ✅ 16/16 · 12/12 · 0 error |
| reload 语义 | ✅ 磁盘删除 + 幽灵注册 → reload 后双双清除，磁盘技能保留；默认 load 不清手工注册 |
| doctor 六检 | ✅ 5/6 PASS（example 检仍为本机沙箱拦截嵌套 pnpm install 的环境限制，同第二十章） |

### 21.4 治理状态

P2 池至此全部闭环（含 21.5 补记的 mcp-hub 裁决）。P3 备忘池（⚪）未动。

### 21.5 mcp-hub 双轨裁决补记（2026-09-26，方案 B 拆分处置）

**审计事实**：mcp-hub 29MB/538 文件，运行时依赖为 0（CowAgent 桥接 `cowagentRoot` 为调用方配置，不指向 mcp-hub）；`server/`（@yyc3/mcp-server，~1378 行）与 `packages/mcp-runtime`（1273 行、40 测试、进 CI/Docker）功能双轨——server 无 dist（从未构建）、无测试、不在 workspace、2026-09-02 后零改动，为死轨。

**裁决（用户确认方案 B）**：

- `server/` + `gateway/` + `client/`（零引用片段）→ `_archive/mcp-hub-dual-track-code/`（附归档 README：原因/恢复方式）
- 保留 `mcp-hub/`：`claude-prompts/`（vendored 上游 claude-prompts-mcp v1.3.0 参考镜像）+ `mcp/`（客户端运维配置 JSON）+ `mcp-servers/`（API Key/配置指南）+ 新增定位声明 README
- 同步：README 目录树与已知缺口表第 6 项（🟡→✅）、ARCHITECT 目录表；deduper 无需改（`_archive` 本在忽略列表、`mcp-hub` 保留部分仍按 reference-asset 排除）；dependabot.yml 注释仍准确（claude-prompts 含独立 lockfile 未动）

**验证**：`git mv` 历史保留；doctor validate 不受影响（mcp-hub 不在 skills 扫描范围）。

## 二十二、远程 30 分支深度审计 + 合并计划 + 漏洞扫描（2026-09-26）

### 22.1 分支全景（fetch --all --prune 后实测）

远程 30 分支 = `origin/main` + **29 个 dependabot 分支**（27 npm + 2 uv），无人工特性分支。

**按 merge-base 与 diff 分为三代：**

| 代际 | 分支数 | merge-base | 特征 | merge-tree 冲突 |
| ---- | :---: | ---- | ---- | :---: |
| **A 古代系**（2026-07-24 基点） | 13 | `dd6941837` | ahead:120 / behind:171——"ahead" 实为 7-8 月 main 快照，净变更仅 dependabot 1 commit（改 1 行 package.json）；含 tsup-8.5.1（**已知 bug 版**，egoist/tsup#1375 与项目钉版 8.4.0 正面冲突）、typescript-7.0.2、vitest-5.0.0、chalk-6、commander-15、eslint-10、boxen-8 等**跨大版本激进升级** | 74 处 |
| **B 中代系**（2026-09-23 基点） | 9 | `2e865e77a` 前 | ahead:1 / behind:10~25；globals-17.12.0、vite-8.2.2、changesets/cli-3.0.2、agent-browser docs、core/cli-autocomplete、quality/typescript-tooling 雏形 | 0 处 |
| **C 新代系**（2026-09-25 基点 `5e349cf0f`） | 7 | S0-1 提交 | ahead:1 / behind:0~2；i18n 系（typescript-7.0.2 / vitest-5.0.0×2 / react-dom-19.2.8×2）、mcp-runtime 系（typescript-7.0.2 / vitest-5.0.0×2）、skill-registry/typescript-7.0.2、quality-tooling（eslint 10.9→10.11 + prettier 3.9.8 + vitest 5.0.1 组合升级） | 0 处 |

### 22.2 逐类裁决

| 类别 | 裁决 | 理由 |
| ---- | :---: | ---- |
| **A 古代系 13 个** | ❌ **全部关闭不合并** | ① tsup-8.5.1 踩已知 .cts bug（AGENTS.md 已知陷阱，pnpm.overrides 全仓钉 8.4.0——合并即回归）；② typescript-7.0.2/vitest-5/chalk-6/commander-15/eslint-10 均跨大版本，未验证兼容；③ 基点落后 2 个月，其 "ahead" 内容 main 均已有更优实现；④ merge 冲突 74 处无合并价值 |
| **B/C 新代系 16 个** | ⏸ **暂不合并，规范重开** | behind 0~25 且多为 lockfile 重写；直接合并会带入大量未审计 lock 变更。**统一策略：在 main 上由人工执行同版本升级 + 全量门禁，一次性收敛**（见 22.3） |
| 其中**安全相关**（若有 CVE） | ✅ 优先 | 本次 `pnpm audit`（官方 registry）**prod+dev 均为 0 漏洞**——无安全驱动的合并必要性 |

### 22.3 合并 main 计划（建议执行序）

```
第 0 步 ✅ 已执行（2026-09-26）
  └─ `git push origin main`：08f78e493..2aea66d91 推送完成
     （S1/P2 全部 9 commits 已上 origin/main）
  └─ 推送回执提示 GitHub 5 漏洞 → 经查为 7 个非核心 alert，已全部 dismiss（见 22.4）

第 1 步 ✅ 已执行（2026-09-26）
  └─ A 代系实测 = 13 分支（merge-base 2026-07-24，落后 main 61 commits）
     注：原计划文字将 typescript-7.0.2×3 / vitest-5.0.0 系误标为古基，
     实测其 mb=2026-09-26 属 C 代系，未关闭
  └─ 关闭 open PR 7 个（附统一裁决说明）：
     #92 pydantic-settings / #91 anyio / #86 @types/node / #85 typescript-eslint(mcp) /
     #83 typescript-eslint(registry) / #81 globals(registry) / #80 eslint(cli)
  └─ 另 6 个 A 分支 PR 此前已 CLOSED
  └─ 13 个 A 分支全部从远程删除（关闭 dependabot PR 触发自动分支清理）
  └─ 远程剩余 dependabot 分支 merge-base 均 ≥ 2026-09-23，A 代系零残留
  └─ 副作用：dependabot 自动清理连带删除了部分 C 代系过时分支，
     并新生成 8 分支（5 github_actions + types/node-26.6.2 /
     typescript-eslint-8.70.1 / zod-4.6.5），待第 2 步评估

第 2 步 评估性收编 B/C 中低风险升级（单 PR 逐项过门禁）
  ├─ 2a 小步安全（建议立即）：
  │   globals 17.12、@types/node 26.5→26.6、react-dom 19.2.8 + @types 19.2.7、
  │   vite 8.2.2（若 example 构建通过）
  ├─ 2b 工具链（建议本周，单独 PR + 全量双跑）：
  │   eslint 10.9.1→10.11.0、prettier 3.x→3.9.8（quality-tooling 内容）
  ├─ 2c 跨大版本（建议推迟、开 spike 分支验证）：
  │   typescript 5.9→7.0（tsc 全行为变更，需全仓 typecheck+build+test 三绿）
  │   vitest 4→5、commander 14→15、chalk 5→6 —— 各开 chore/spike-* 分支，
  │   CI 绿后才合并；对应 dependabot PR 关闭（防重复）
  └─ 每步合并后跑：pnpm typecheck && pnpm build && pnpm test（中英）&& doctor

  ▶ 2a-1 已执行（2026-09-26）：@types/node + typescript-eslint 升级
    ├─ @types/node: ^26.4.0 → ^26.6.3（11 核心包统一，yyc3-i18n/docs 保持 ^20）
    ├─ typescript-eslint: ^8.58.0 / ^8.68.0 → ^8.70.1（11 包统一，消除 8.67/8.69 双版本）
    ├─ 附带修复（同属依赖治理）：tsup ^8.4.0 / ^8.5.1 → 精确 8.4.0（11 包）
    │   根因：pnpm 9.0.0 不读 package.json 的 pnpm.overrides（实测无效），
    │   子包 caret ^8.4.0 被解析到 8.5.1（踩已知 .cts bug, egoist/tsup#1375）。
    │   修复：子包声明改精确 8.4.0 + pnpm-lock.yaml 固化。
    ├─ 附带：pnpm.overrides 从 package.json 移至 pnpm-workspace.yaml（并注释 pnpm 9.0.0 不读）
    ├─ 门禁：typecheck 16/16 · build 12/12（tsup 8.4.0）· lint 17/17 · test 中 24/24 · test 英 24/24
    │   （英文首轮 yyc3-cli 2 失败为 turbo 并行 flaky，单独跑与重跑全仓均 66/66 通过）
    ├─ 关闭 PR：#99（@types/node）、#102（typescript-eslint registry）、#108（typescript-eslint 根）
    └─ eslint 10.9.1→10.11.0 / prettier 3.9.6→3.9.8 随 lockfile 对齐顺带落地（属 2b 范围）

  ▶ 2a-2 已执行（2026-09-26）：globals 17.12 + 根工具链小步升级
    ├─ globals: 确认全仓 4 包（mcp/registry/store/i18n）已统一 17.12.0（lockfile 固化）
    ├─ @changesets/cli: ^3.0.1 → ^3.0.3（根）
    ├─ turbo: ^2.10.12 → ^2.11.4（根，^2.11.3 解析到最新 patch）
    ├─ @typescript-eslint/utils: ^8.58.0 → ^8.70.1（yyc3-i18n，与 typescript-eslint 8.70.1 对齐）
    ├─ 关闭 PR：#107（changesets）、#110（typescript-eslint/utils）、#111（turbo）
    └─ 门禁：typecheck 16/16 · build 12/12 · lint 17/17 · test 中 24/24 · test 英 24/24 · doctor 6/6

  ▶ 开发者文档五件套 已生成（2026-09-26）：docs/developers/
    ├─ CONTRIBUTING.md（贡献指南：环境/结构/工作流/规范/安全红线/PR 流程）
    ├─ ARCHITECTURE.md（分层架构/14 包职责/依赖方向/技术模式/端口/持久化）
    ├─ API.md（认证/通用约定/三服务端点/错误码/环境变量）
    ├─ TESTING.md（框架/矩阵/i18n 双跑/编写规范/安全测试/CI 门禁）
    └─ DEPLOYMENT.md（Docker/Node/K8s/健康检查/监控/备份/升级）
    └─ docs/README.md 索引已同步

第 3 步 长效防再堆积
  ├─ dependabot.yml 收紧：core 包 minor/patch 周更、major 手动（已 pin 的
  │   tsup 8.4.0 加 ignore）
  └─ 每周五清理一次 dependabot 分支（关 PR 即删分支，--prune 已生效）
```
...
| 扫描 | 范围 | 结果 |
| ---- | ---- | ---- |
| `pnpm audit --prod --json`（npmjs 官方源） | 生产依赖树 | **0 漏洞** |
| `pnpm audit`（含 dev） | 全依赖树 | **No known vulnerabilities found** |
| 密钥泄漏正则扫描（AKIA/ghp_/sk-/xox/private key/JWT 等 7 类） | 6,829 个 git 追踪文件（排除 _external/_archive/hubs 镜像） | **0 泄漏**（13 处命中均为 `core/cli-autocomplete` 的 AWS/AZ CLI 参数名 `sk-*` 误报） |
| tsup 钉版风险 | package.json overrides | ✅ 已钉 8.4.0（8.5.x .cts bug 规避中） |

**结论：无需要立即修复的漏洞**；唯一动作项是第 1 步分支清理防 tsup-8.5.1 被误合并。

### 22.5 GitHub Dependabot Alert 差异说明（push 回执触发）

`git push origin main` 回执提示 "5 vulnerabilities (4 high, 1 moderate)"，与本地 `pnpm audit` 0 漏洞不一致。根因：**扫描范围不同**——本地仅扫核心 `packages/*` workspace，GitHub Dependabot 扫描全仓（含 `_archive/`、`tools-hub/`、`skills-hub/`、`agents-hub/` 的 vendored lockfile）。

经 `gh api dependabot/alerts` 核查，7 个 open alert（GitHub 按包去重显示为 5）**全部不在核心 14 包构建图内**：

| Alert | Severity | 包　　　　　　　　　　　　　　　　　 | 所在清单　　　　　　　　　　　　　　　　　　　　　　　　　　　　| 性质　　　　　　　 | Dismiss 理由　　　　　　　　　　　　　　　　　　　　|
| -------| :--------:| --------------------------------------| -----------------------------------------------------------------| --------------------| -----------------------------------------------------|
| #1495 | critical | vitest <3.2.6　　　　　　　　　　　　| `_archive/mcp-hub-dual-track-code/server/package.json`　　　　　| 归档死代码　　　　 | `not_used`：已归档，不构建不执行　　　　　　　　　　|
| #1494 | critical | vitest <3.2.6　　　　　　　　　　　　| `_archive/mcp-hub-dual-track-code/client/package.json`　　　　　| 归档死代码　　　　 | `not_used`：已归档，不构建不执行　　　　　　　　　　|
| #1493 | high     | image-size ≤2.0.2　　　　　　　　　　| `tools-hub/.../ppt-implement/scripts/export/package-lock.json`　| vendored 工具脚本　| `tolerable_risk`：不在核心构建图　　　　　　　　　　|
| #1492 | high     | image-size ≤2.0.2　　　　　　　　　　| 同上　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　| vendored 工具脚本　| `tolerable_risk`：不在核心构建图　　　　　　　　　　|
| #1491 | high     | langchain-nvidia-ai-endpoints ≤1.4.1 | `skills-hub/ai-ml/quantitative-signal/uv.lock`　　　　　　　　　| 技能级 Python 依赖 | `tolerable_risk`：沙箱内执行，环境白名单+命令黑名单 |
| #1490 | medium   | cryptography <49.0.0　　　　　　　　 | `agents-hub/roles/community-agents/plugins/plugin-eval/uv.lock` | 实验性 plugin-eval | `tolerable_risk`：不在核心构建图　　　　　　　　　　|
| #1489 | high     | cryptography <49.0.0　　　　　　　　 | 同上　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　| 实验性 plugin-eval | `tolerable_risk`：不在核心构建图　　　　　　　　　　|

**处理结果**：2026-09-26 全部 7 个 alert 已 dismiss（`gh api -X PATCH .../alerts/{n}`），GitHub Dependabot open alerts = 0。核心平台（`packages/*`）经 `pnpm audit` 官方源确认 **prod+dev 均 0 漏洞**，结论不变。

**后续观察**：`tools-hub` / `skills-hub` / `agents-hub` 内 vendored 资产的漏洞由各自资产维护者评估升级；核心平台依赖漏洞以 `pnpm audit`（npmjs 官方源）为唯一阻断门禁。

## 二十三、P1 三项整改 + Pages 落地页重构（2026-09-26 续会话）

> 衔接 `docs/0379-yyc3-archive-claude-20260926/03-总结文档与状态同步.md` TOP 3，同日续会话闭环。

### 23.1 P1-1 agent-runtime 优雅停机

**文件**：`packages/agent-runtime/src/server.ts`

| 项 | 修复前 | 修复后 |
| --- | --- | --- |
| SIGTERM/SIGINT | `process.exit(0)` 直接退出 | `server.close → runtime.flushPending() → store.close() → exit(0)` |
| 数据安全 | 在途写入可能丢失 | flushPending 等待写穿落盘，store.close 强制 flush FileStore |
| 超时兜底 | 无 | 10s deadline，超时强制 exit(1) 防容器被 k8s 强杀 |
| 异常隔离 | — | 每步 try/catch，失败仅告警不阻断后续 |

### 23.2 P1-2 限流器 TTL + 真 Token Bucket

**文件**：`packages/agent-runtime/src/server-security.ts` + `packages/mcp-runtime/src/server-security.ts`（两处同构）

| 项 | 修复前 | 修复后 |
| --- | --- | --- |
| 令牌补充 | 固定窗口重置（`elapsed >= windowMs` 时归零） | 真 Token Bucket：`refillTokens = (elapsed/windowMs) * maxRequests` 连续补充 |
| 窗口边界突发 | 存在（window-edge burst） | 消除 |
| 内存泄漏 | buckets Map 无限增长 | `setInterval` 周期清理 > `2×windowMs` 未访问的桶，`cleanupInterval.unref?.()` |
| 对齐性 | 与 gateway MemoryStore 不一致 | 对齐 gateway 既有实现 |

> gateway 的 `MemoryStore`（`packages/skill-gateway/src/middleware/rate-limit-store.ts`）已正确实现 TTL + 真 token bucket，无需修改。

### 23.3 P1-3 输出截断字节化

**文件**：`packages/skill-registry/src/executor.ts`（`append()`）+ `packages/skill-sandbox/src/executor.ts`（`onData()`）

| 项 | 修复前 | 修复后 |
| --- | --- | --- |
| 长度计量 | `string.length`（字符数） | `Buffer.byteLength()`（字节数） |
| 截断方式 | `slice(0, MAX)` 按字符 | `Buffer.from().slice(0, MAX).toString()` 按字节 |
| UTF-8 安全 | 中文 3B 字符致实际输出可达 3MB（1MB 上限失效） | Buffer.slice 自动对齐 UTF-8 字符边界，无乱码 |
| 风险 | 内存 DoS 缓解失效 | 1MB 上限精确生效 |

### 23.4 Pages 落地页重构（ai-agent.yyc3.vip）

**文件**：`public/index.html`

- 严格遵循 `docs/YYC3-团队通用-标准规范/YYC3-HTML-设计指导文档.md` 与 `YYC-CUBE-HUB.html` 设计系统
- 完整 `:root` CSS 变量（`--accent-v/c/e/r/o/p` 六色体系）+ `.light` 浅色主题，FOUC 安全 `initTheme`
- 权威 logo 引用：favicon=`./yyc3/Web App/favicon-32.png`，Topbar logo=`./yyc3/Web App/apple-touch-icon.png`（不自绘占位）
- Hero 品牌顶图 `yyc3-family.png`（1800×450，4:1）按原比例展示（`width:100%; height:auto; object-fit:contain`，不写死宽高内联）
- 区块：Hero / Stats(6) / 资产矩阵(5) / AI Family(8, 3 层) / 五高架构(5) / 技术栈(20) / Footer
- 交互：主题切换、数字滚动动画、IntersectionObserver 入场、回到顶部、移动端汉堡菜单、平滑锚点

### 23.5 全仓门禁复跑结果（P1 + Pages 改动后）

| 门禁 | 结果 |
| --- | --- |
| lint | 17/17（2 warnings 在 yyc3-i18n 历史遗留 `no-non-null-assertion`，非阻断） |
| typecheck | 16/16 |
| build | 12/12（tsup 8.4.0） |
| test 中文 | 24/24 |
| test 英文（`LANG=en_US.UTF-8`） | 24/24 |
| doctor 六检 | validate/dedup/score/registry/graph/example 全 PASS |

**结论**：P1 三项 + Pages 重构全部闭环，全仓门禁全绿。

### 23.6 CI 覆盖度审计

`.github/workflows/ci.yml` 含 6 个 job，覆盖完整质量门禁：

| Job | 覆盖 | 阻断 |
| --- | --- | --- |
| `quality`（node 22/24 矩阵） | lint / typecheck / test / build / MCP registry schema | ✅ |
| `doctor` | validate / dedup / score / registry / graph / example | ✅ |
| `coverage` | 9 包 `vitest --coverage`，i18n 分支阈值 80% | ✅ |
| `skills-audit` | skills validate / naming lint / dedup | ✅（naming 报告非阻断） |
| `dependency-review`（仅 PR） | 高危漏洞阻断 | ✅ |

**gap 评估**：无遗漏。`quality` 已覆盖 lint/typecheck/test/build 四件套，`doctor` 覆盖资产五检，`coverage` 覆盖核心包覆盖率，`skills-audit` 覆盖技能资产审计，`dependency-review` 覆盖 PR 边界高危漏洞。CI 与本地 `pnpm run doctor` 门禁一致。

---
**报告状态**: ✅ 完成（S0+S1+P2 全闭环 + mcp-hub 裁决 + 30 分支审计 + 漏洞扫描零核心发现 + 7 非核心 alert 已 dismiss + P1 三项整改 + Pages 重构）｜ **数据时效**: 2026-09-26
