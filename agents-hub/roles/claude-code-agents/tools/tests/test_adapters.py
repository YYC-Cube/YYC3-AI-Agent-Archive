...
class TestCodexAdapter:
    ...
    def test_strips_claude_only_skill_fields(self, tmp_path: Path, output_root: Path):
        ...
        plugin_dir.mkdir()
        (plugin_dir / ".ai-family-plugin").mkdir()
        (plugin_dir / ".ai-family-plugin" / "plugin.json").write_text('{"name": "demo"}')
        skill = _make_skill(
            plugin_dir,
            "noisy",
    ...
    def test_plugin_manifest_description_falls_back_to_name(
        ...
        plugin_dir.mkdir()
        (plugin_dir / ".ai-family-plugin").mkdir()
        (plugin_dir / ".ai-family-plugin" / "plugin.json").write_text(
...
class TestOpenCodeAdapter:
    ...
    def test_rejects_invalid_opencode_skill_id(self, tmp_path: Path):
        ...
        plugin_dir.mkdir()
        (plugin_dir / ".ai-family-plugin").mkdir()
        (plugin_dir / ".ai-family-plugin" / "plugin.json").write_text('{"name": "bad_plugin"}')
        skill = _make_skill(
            plugin_dir,
            "hello",
            "name: hello\ndescription: Use when testing.",
...
class TestLoadPlugin:
    def test_rejects_double_underscore_plugin_name(self, tmp_path: Path, monkeypatch):
        ...
        (bad_plugin / ".ai-family-plugin").mkdir(parents=True)
        (bad_plugin / ".ai-family-plugin" / "plugin.json").write_text('{"name": "bad__name"}')
