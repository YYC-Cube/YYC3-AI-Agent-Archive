...
class TestMarketplaceConsistency:
    def test_every_local_marketplace_entry_resolves_to_plugin_dir(self):
        mp = json.loads((WORKTREE / ".ai-family-plugin" / "marketplace.json").read_text())
        for entry in mp.get("plugins", []):
            ...
            if isinstance(source, str) and source.startswith("./plugins/"):
                ...
                assert (plugin_path / ".ai-family-plugin" / "plugin.json").is_file(), (
                    f"{entry['name']}: missing .ai-family-plugin/plugin.json"
                )
    ...
    def test_marketplace_version_matches_per_plugin_json(self):
        """Each marketplace.json entry's version must match plugins/<name>/.ai-family-plugin/plugin.json."""
        mp = json.loads((WORKTREE / ".ai-family-plugin" / "marketplace.json").read_text())
        mismatches = []
        for entry in mp.get("plugins", []):
            source = entry.get("source")
            ...
            pj_path = WORKTREE / source.removeprefix("./") / ".ai-family-plugin" / "plugin.json"
...
class TestPluginJsonIntegrity:
    ...
    def test_plugin_json_has_name_and_version(self, plugin_name: str):
        pj_path = PLUGINS_DIR / plugin_name / ".ai-family-plugin" / "plugin.json"
