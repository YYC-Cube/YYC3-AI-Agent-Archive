...
class CodexAdapter(HarnessAdapter):
    ...
    def _emit_codex_plugin_manifest(self, plugin: PluginSource, result: EmitResult) -> None:
        """Write `plugins/<plugin>/.codex-plugin/plugin.json`, next to `.ai-family-plugin/`.
    ...
    def _codex_plugin_manifest(self, plugin: PluginSource) -> dict:
        ...
        `.ai-family-plugin/plugin.json` omits it (e.g. `plugin-eval`): the
    ...
    def _read_marketplace_root(self) -> dict:
        """Read the source `.ai-family-plugin/marketplace.json` for name/owner/metadata."""
        path = self.repo_root / ".ai-family-plugin" / "marketplace.json"
        if not path.is_file():
            return {}
        try:
