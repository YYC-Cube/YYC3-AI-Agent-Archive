...
2. Emits `.cursor-plugin/marketplace.json` at root mirroring `.ai-family-plugin/marketplace.json`
...
def _read_marketplace_root(source_root: Path = WORKTREE) -> dict:
    ...
    """
    path = source_root / ".ai-family-plugin" / "marketplace.json"
    try:
...
class CursorAdapter(HarnessAdapter):
    ...
    def _build_marketplace(self, plugins: list[PluginSource]) -> dict:
        """Root .cursor-plugin/marketplace.json — mirror of .ai-family-plugin/ shape with
        ...
        candidate = self.output_root / ".ai-family-plugin" / "marketplace.json"
