...
MARKETPLACE_JSON = WORKTREE / ".ai-family-plugin" / "marketplace.json"
    ...
    if gemini_commands.is_dir():
        for toml_path in gemini_commands.rglob("*.toml"):
            ...
            if toml_path.parent == gemini_commands:
                ...
                src = PLUGINS_DIR / plugin / ".ai-family-plugin" / "plugin.json"
                if src.is_file():
                    pairs.append((src, toml_path))
            else:
...
def check_marketplace_consistency(report: Report) -> None:
    ...
    for name in sorted(actual - listed_all):
        ...
        # local .ai-family-plugin/plugin.json.
        if (PLUGINS_DIR / name / ".ai-family-plugin" / "plugin.json").is_file():
            report.add(
