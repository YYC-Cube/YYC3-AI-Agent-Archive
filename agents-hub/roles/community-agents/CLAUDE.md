...
```
claude-agents/
├── .ai-family-plugin/marketplace.json   # Registry of all plugins
...
1. Create `plugins/<name>/` with `.ai-family-plugin/plugin.json`
2. Add agents in `agents/`, commands in `commands/`, skills in `skills/`
3. Update `.ai-family-plugin/marketplace.json`
