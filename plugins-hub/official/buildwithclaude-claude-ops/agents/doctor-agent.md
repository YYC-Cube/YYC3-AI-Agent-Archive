...
The `repository` field in `.ai-family-plugin/plugin.json` is an object but must be a string.
...
- If unfixable, restore from git: `git checkout -- .ai-family-plugin/plugin.json`
...
for ver_dir in ~/.claude/plugins/cache/ops-marketplace/ops/*/; do
  cp "$PLUGIN_ROOT/.ai-family-plugin/plugin.json" "$ver_dir/.ai-family-plugin/plugin.json" 2>/dev/null || true
done
```
