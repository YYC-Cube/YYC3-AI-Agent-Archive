...
# description: 版本号批量更新脚本 · 跨文件同步版本号并检测版本漂移
# description-en: Version batch update script · Synchronize version numbers across files and detect version drift
# author: YanYuCloudCube Team <admin@0379.email>
# version: v1.0.0
# created: 2026-04-29
# updated: 2026-04-29
# status: active
# tags: [script],[versioning],[devops],[automation]
#
# copyright: YanYuCloudCube Team
# license: MIT
#
# brief: 批量更新项目中所有声明文件的版本号，支持漂移检测和仓库级审计
# brief-en: Batch update version numbers across all declared files, support drift detection and repo-wide audit
#
# details:
# - 读取 .version-bump.json 配置获取需要更新的文件列表
# - 支持三种模式：更新版本、检查当前版本、全面审计
# - 自动检测遗漏的旧版本字符串（grep 仓库）
# - 支持嵌套 JSON 字段路径（jq 路径转换）
# - 输出详细的变更报告和漂移警告
#
# details-en:
# - Read .version-bump.json config for files to update
# - Support three modes: bump version, check current, full audit
# - Auto-detect missed old version strings (grep repo)
# - Support nested JSON field paths (jq path conversion)
# - Output detailed change report and drift warnings
#
# usage:
#   bump-version.sh <new-version>   更新所有声明文件到新版本
#   bump-version.sh --check         报告当前版本（检测漂移）
#   bump-version.sh --audit         检查 + grep 仓库查找旧版本字符串
#
# dependencies: bash 4+, jq, .version-bump.json
# notes: 用于发布流程的版本管理 / Used for release workflow version management
# =============================================================================
