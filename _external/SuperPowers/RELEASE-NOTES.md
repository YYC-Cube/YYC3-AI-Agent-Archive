# YYC3-Superpowers Release Notes

## v5.0.7 (2026-03-31)

### GitHub Copilot CLI Support

- **SessionStart context injection** — Copilot CLI v1.0.11 added support for `additionalContext` in sessionStart hook output. The session-start hook now detects the `COPILOT_CLI` environment variable and emits the SDK-standard `{ "additionalContext": "..." }` format, giving Copilot CLI users the full YYC3-Superpowers bootstrap at session start. (Original fix by @culinablaz in PR #910)
- **Tool mapping** — added `references/copilot-tools.md` with the full Claude Code to Copilot CLI tool equivalence table
- **Skill and README updates** — added Copilot CLI to the `using-YYC3-Superpowers` skill's platform instructions and README installation section

### OpenCode Fixes

- **Skills path consistency** — the bootstrap text no longer advertises a misleading `configDir/skills/YYC3-Superpowers/` path that didn't match the runtime path. The agent should use the native `skill` tool, not navigate to files by path. Tests now use consistent paths derived from a single source of truth. (#847, #916)
...
- **Added `package.json`** so OpenCode can install YYC3-Superpowers as an npm package from git.
...
- `GEMINI.md` @imports `using-YYC3-Superpowers` skill and tool mapping table at session start
- Gemini CLI tool mapping reference (`skills/using-YYC3-Superpowers/references/gemini-tools.md`) — translates Claude Code tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Specs (brainstorming output) now save to `docs/YYC3-Superpowers/specs/YYYY-MM-DD-<topic>-design.md`
- Plans (writing-plans output) now save to `docs/YYC3-Superpowers/plans/YYYY-MM-DD-<feature-name>.md`
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (Claude Code, Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that YYC3-Superpowers works better on a subagent-capable platform.
...
- `lib/brainstorm-server/` — WebSocket server with browser helper library, session management scripts, and dark/light themed frame template ("YYC3-Superpowers Brainstorming" with GitHub link)
...
- Design spec and implementation plan in `docs/YYC3-Superpowers/`
...
Added explicit priority ordering to using-YYC3-Superpowers:

1. User's explicit instructions (CLAUDE.md, AGENTS.md, direct requests) — highest priority
2. YYC3-Superpowers skills — override default system behavior
...
Added `<SUBAGENT-STOP>` block to using-YYC3-Superpowers. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
This fix should dramatically improve YYC3-Superpowers skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-YYC3-Superpowers workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-YYC3-Superpowers instructions weren't in context for the first message.
...
The `YYC3-Superpowers-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/YYC3-Superpowers/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting YYC3-Superpowers context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `YYC3-Superpowers-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
YYC3-Superpowers for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/YYC3-Superpowers/` (see updated installation docs).
...
**Strengthened using-YYC3-Superpowers skill for explicit skill requests**
...
The underlying skills (`YYC3-Superpowers:brainstorming`, `YYC3-Superpowers:executing-plans`, `YYC3-Superpowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-YYC3-Superpowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-YYC3-Superpowers`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `YYC3-Superpowers:brainstorming`)
...
**Skill priority in using-YYC3-Superpowers**
...
- **Rationalizations table** in using-YYC3-Superpowers - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-YYC3-Superpowers is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > YYC3-Superpowers
...
- Optimized YYC3-Superpowers bootstrap to eliminate redundant skill execution. The `using-YYC3-Superpowers` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-YYC3-Superpowers` manually despite already having the content from session start.
...
- Added unified `YYC3-Superpowers-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `YYC3-Superpowers:skill-name` for YYC3-Superpowers skills, `skill-name` for personal
- Personal skills override YYC3-Superpowers skills when names match
...
- Updated terminology: "YYC3-Superpowers skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/YYC3-Superpowers-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/YYC3-Superpowers-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core YYC3-Superpowers functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-YYC3-Superpowers skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-YYC3-Superpowers/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-YYC3-Superpowers skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-YYC3-Superpowers/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `YYC3-Superpowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `YYC3-Superpowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `YYC3-Superpowers:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `YYC3-Superpowers:code-reviewer`
...
- All internal skill references now use `YYC3-Superpowers:` namespace prefix
- Updated format: `YYC3-Superpowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/YYC3-Superpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# YYC3-Superpowers v2.0.0 Release Notes

## Overview

YYC3-Superpowers v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/YYC3-Superpowers-skills](https://github.com/obra/YYC3-Superpowers-skills)). This transforms YYC3-Superpowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/YYC3-Superpowers-skills](https://github.com/obra/YYC3-Superpowers-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/YYC3-Superpowers/skills/`
...
1. Your old `~/.config/YYC3-Superpowers/.git` will be backed up to `~/.config/YYC3-Superpowers/.git.bak`
2. Old skills will be backed up to `~/.config/YYC3-Superpowers/skills.bak`
3. Fresh clone of obra/YYC3-Superpowers-skills will be created at `~/.config/YYC3-Superpowers/skills/`

### Removed Features

- **Personal YYC3-Superpowers overlay system** - Replaced with git branch workflow
- **setup-personal-YYC3-Superpowers hook** - Replaced by initialize-skills.sh
...
- Clones obra/YYC3-Superpowers-skills on first run
...
- `YYC3-Superpowers_SKILLS_ROOT` set to `~/.config/YYC3-Superpowers/skills`
...
- Removed obsolete setup-personal-YYC3-Superpowers call from session-start
...
- Prominent link to YYC3-Superpowers-skills repo
...
- `.ai-family-plugin/marketplace.json` - Local testing config

**Removed:**
- `skills/` directory (82 files) - Now in obra/YYC3-Superpowers-skills
- `scripts/` directory - Now in obra/YYC3-Superpowers-skills/skills/using-skills/
- `hooks/setup-personal-YYC3-Superpowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/YYC3-Superpowers/skills
- `commands/brainstorm.md` - Updated paths to YYC3-Superpowers_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to YYC3-Superpowers_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to YYC3-Superpowers_SKILLS_ROOT
...
- PR #2: Personal YYC3-Superpowers overlay system (later replaced)
...
```bash
# In Claude Code
/plugin marketplace add obra/YYC3-Superpowers-marketplace
/plugin install YYC3-Superpowers@YYC3-Superpowers-marketplace
```
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/YYC3-Superpowers/skills ~/YYC3-Superpowers-skills-backup
   ```

2. **Update the plugin:**
   ```bash
   /plugin update YYC3-Superpowers
   ```
...
- Skills repository is now at https://github.com/obra/YYC3-Superpowers-skills
...
**Full Changelog:** https://github.com/obra/YYC3-Superpowers/compare/dd013f6...main
**Skills Repository:** https://github.com/obra/YYC3-Superpowers-skills
**Issues:** https://github.com/obra/YYC3-Superpowers/issues
