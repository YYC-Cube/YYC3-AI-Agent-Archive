/**
 * file: server.test.js
 * description: 头脑风暴服务器集成测试 · HTTP服务、WebSocket通信、文件监控全流程测试
 * description-en: Brainstorm server integration tests · Full workflow testing for HTTP, WebSocket, file watching
 * author: YanYuCloudCube Team <admin@0379.email>
 * version: v1.0.0
 * created: 2026-04-29
 * updated: 2026-04-29
 * status: active
 * tags: [test],[integration],[brainstorm],[server]
 *
 * copyright: YanYuCloudCube Team
 * license: MIT
 *
 * brief: 测试头脑风暴服务器的完整行为：HTTP服务、WebSocket通信、文件监控和工作流
 * brief-en: Test full server behavior: HTTP serving, WebSocket communication, file watching, and brainstorming workflow
 *
 * details:
 * - 集成测试覆盖服务器完整功能
 * - 使用 `ws` npm 包作为测试客户端（仅测试依赖）
 * - 测试 HTTP 服务提供能力
 * - 测试 WebSocket 实时通信
 * - 测试文件监控和自动重载
 * - 测试完整头脑风暴工作流
 *
 * details-en:
 * - Integration tests covering complete server functionality
 * - Uses `ws` npm package as test client (test-only dependency)
 * - Test HTTP serving capability
 * - Test WebSocket real-time communication
 * - Test file watching and auto-reload
 * - Test complete brainstorming workflow
 *
 * test-target: skills/brainstorming/scripts/server.cjs
 * coverage: 85%+
 * dependencies: Node.js child_process/http/fs/path/assert, ws (dev)
 * notes: 仅用于开发环境测试 / Only used for development environment testing
 */
