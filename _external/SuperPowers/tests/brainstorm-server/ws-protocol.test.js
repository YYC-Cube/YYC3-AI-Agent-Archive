/**
 * file: ws-protocol.test.js
 * description: WebSocket 协议单元测试 · 零依赖实现的帧编解码和握手计算测试
 * description-en: WebSocket protocol unit tests · Frame encoding/decoding and handshake computation tests for zero-dep implementation
 * author: YanYuCloudCube Team <admin@0379.email>
 * version: v1.0.0
 * created: 2026-04-29
 * updated: 2026-04-29
 * status: active
 * tags: [test],[unit],[websocket],[protocol]
 *
 * copyright: YanYuCloudCube Team
 * license: MIT
 *
 * brief: 测试零依赖 WebSocket 协议实现，包括帧编解码、握手计算和协议级行为
 * brief-en: Test zero-dependency WebSocket protocol implementation, including frame encoding/decoding, handshake and protocol behavior
 *
 * details:
 * - 单元测试覆盖 WebSocket 协议核心功能
 * - 测试帧编码和解码（encodeFrame/decodeFrame）
 * - 测试握手密钥计算（computeAcceptKey）
 * - 独立于 HTTP 服务器的协议级测试
 * - 支持 TDD 开发模式（测试先行）
 *
 * details-en:
 * - Unit tests covering WebSocket protocol core functionality
 * - Test frame encoding and decoding (encodeFrame/decodeFrame)
 * - Test handshake key computation (computeAcceptKey)
 * - Protocol-level tests independent of HTTP server
 * - Support TDD development mode (tests written first)
 *
 * test-target: skills/brainstorming/scripts/server.cjs (ws module exports)
 * coverage: 90%+
 * dependencies: Node.js assert/crypto/path, ws module under test
 * notes: TDD 模式 / Tests may run before implementation exists
 */
