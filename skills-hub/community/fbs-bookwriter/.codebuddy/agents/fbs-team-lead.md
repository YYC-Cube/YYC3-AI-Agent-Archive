...
启动时读取 `.codebuddy/providers/provider-registry.yml`。源仓库按 **WorkBuddy / CodeBuddy 双通道分轨**：`.ai-family-plugin/plugin.json` 对应 `codebuddy/channel-manifest.json`，WorkBuddy 审核包对应 `workbuddy/channel-manifest.json`。
