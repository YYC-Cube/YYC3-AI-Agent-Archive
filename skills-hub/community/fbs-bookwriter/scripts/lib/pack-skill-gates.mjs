...
export function runPackSkillGates(options = {}) {
  ...
  const plugin = JSON.parse(fs.readFileSync(path.join(root, '.ai-family-plugin/plugin.json'), 'utf8'));
  ...
  const versionTargets = [
    ...
    ['.ai-family-plugin/plugin.json', plugin.version],
  ];
  ...
  if (plugin.name !== 'fbs-bookwriter') {
    throw new Error('.ai-family-plugin/plugin.json name 应为 fbs-bookwriter');
  }
