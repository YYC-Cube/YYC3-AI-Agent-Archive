...
Verify: `cat ~/.claude/plugins/cache/last30days-skill/last30days/*/.ai-family-plugin/plugin.json | grep version` returns `"version": "3.0.9"`.
...
- **`/last30days` slash command now registers on Claude Code v2.1.105+.** `.ai-family-plugin/plugin.json` declared `"skills": ["./"]`, which newer Claude Code rejects with `Path escapes plugin directory: ./ (skills)`. The skill silently failed to register, so `/last30days <query>` returned "Unknown command" even though `/plugin list` showed the plugin as installed. Fix: `"skills": ["skills"]` so the loader scans the real skill subdirectory.
- **Version drift between manifests.** `.ai-family-plugin/marketplace.json` was pinned to `3.0.0` while `.ai-family-plugin/plugin.json` advertised `3.0.1`. The `/plugin` resolver used the marketplace version and could install stale cached metadata alongside the correct build. Both manifests now agree on `3.0.2`.
...
- Marketplace plugin support via `.ai-family-plugin/plugin.json` (inspired by @galligan, [#1](https://github.com/mvanhorn/last30days-skill/pull/1))
