...
- Manifest-driven configuration in `.ai-family-plugin/plugin.json`
...
1. **Manifest location**: The `plugin.json` manifest MUST be in `.ai-family-plugin/` directory
2. **Component locations**: All component directories (commands, agents, skills, hooks) MUST be at plugin root level, NOT nested inside `.ai-family-plugin/`
3. **Optional components**: Only create directories for components the plugin actually uses
4. **Naming convention**: Use kebab-case for all directory and file names

## Plugin Manifest (plugin.json)

The manifest defines plugin metadata and configuration. Located at `.ai-family-plugin/plugin.json`:
...
1. **Plugin manifest**: Reads `.ai-family-plugin/plugin.json` when plugin enables
...
- Confirm directories are at plugin root (not in `.ai-family-plugin/`)
