...
try:
  ...
  assert (Path(root)/'.ai-family-plugin'/'plugin.json').exists()
  print(root)
...
Glob(path: ~/.codebuddy/plugins/marketplaces): **/security-scan/.ai-family-plugin/plugin.json

# 搜索2：directory 类型市场（从 known_marketplaces.json 读取 installLocation）
# 对每个 directory 类型市场执行：
Glob(path: {installLocation}): **/security-scan/.ai-family-plugin/plugin.json
```

从匹配结果中提取插件根目录：取 `plugin.json` 路径去掉尾部 `/.ai-family-plugin/plugin.json` 即为插件根目录。
...
```
插件根目录解析失败：未找到 security-scan/.ai-family-plugin/plugin.json。
...
3. 确认 installLocation 下 plugins/security-scan/.ai-family-plugin/plugin.json 文件存在
```
