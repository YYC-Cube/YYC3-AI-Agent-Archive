...
def _normalize_years(years):
    """Use the newest covered data year when the requested year is not published yet."""
    if not years:
        years = [str(min(date.today().year, LATEST_KNOWN_YEAR))]
    ...
    for year in years:
        ...
        normalized.append(str(min(year_int, LATEST_KNOWN_YEAR)))
    ...
    for place in place_list:
        for year in args.year:
            for classify in args.classify:
                ...
                # records  = fetch_data(year, place, None, read_key="lingxi_doc")
