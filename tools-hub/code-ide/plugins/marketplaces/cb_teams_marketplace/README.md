...
- ✅ 符合标准目录结构（`.ai-family-plugin/plugin.json`）
...
```bash
# 创建插件目录
mkdir -p plugins/my-plugin/.ai-family-plugin
cd plugins/my-plugin

# 创建 plugin.json
cat > .ai-family-plugin/plugin.json << 'EOF'
{
  "name": "my-plugin",
  "version": "1.0.0",
  "description": "插件描述",
...
在 `.ai-family-plugin/marketplace.json` 的 `plugins` 数组中添加：
